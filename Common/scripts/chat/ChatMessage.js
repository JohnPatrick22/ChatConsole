function ChatMessage(id, messageId, messageType, messageDirection, messageContent, originatorType, originatorEmail, originatorName, destinators, formatType) {
	...
	this.getOriginatorNameForAgent = function() {
		var value = '';
		
		if (message.originatorType == MessageTypes.FROM_AGENT) {
			value = 'From agent ' + message.originatorName; 
		} else if (message.originatorType == MessageTypes.FROM_CLIENT) {
			value = 'From client ' + message.originatorName; 
		} else if (message.originatorType == MessageTypes.FROM_SERVER) {
			value = 'From server';
		} else {
			value = message.originatorType;		
		}
