...
function ChatEventHandler() {
	...
	var lastMessageOriginatorType=null;
...
//		window.location.replace("../SurveyConsole/index.jsp" + url);
	...
	function moveToSurvey() {
		...
		window.location.replace("../SurveyConsole/survey.html?" + url);
	};
	...
	function setMessageToDisplay(message) {
        ...
        if (lastMessageFrom == message.getOriginatorName() && lastMessageOriginatorType == message.getOriginatorType()) {
		...
		messageContainer.append(item);
   		...
   		if (message.getOriginatorType() != MessageTypes.FROM_TYPING) {
   			...
   			lastMessageOriginatorType = message.getOriginatorType();
   		}
