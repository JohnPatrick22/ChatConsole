...
function ChatMessageHandler() {
	...
	function handleTransferToGroupMessage(jsonMessage) {
		...
		gChatEventHandler.handleNewChatMessage(new ChatMessage(chatId, -10, jsonMessage.msgType, MessageTypes.IN, "You have been transferred to the group " + groupName, MessageTypes.FROM_SERVER, null, "Server", new Array(), MessageTypes.MSG_TEXT));
	};
	...
	function handleAgentChatTransferMessage(jsonMessage) {
		...
		var msgFrom = "Agent " + agentFrom.getName() + " transferred the chat to " + agentTo.getName();
